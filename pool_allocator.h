...
struct Pool_Allocator pool_allocator_init(void* mem, size_t block_size, size_t nr_blocks){
  #ifdef DBG
    assert((mem != NULL));
    assert(!(block_size < sizeof(struct Pool_Memory_Block)));
    assert(!(nr_blocks != 0 && block_size > SIZE_MAX / nr_blocks));
    //assert(!((uintptr_t)mem % block_size != 0));
  ...
  //if((uintptr_t)mem % block_size != 0) return allocator;
...
void* pool_allocator_alloc(struct Pool_Allocator* allocator){
    ...
    assert(!(allocator->base == (uintptr_t)NULL));
    assert(allocator->head != NULL);
  #endif
  if(allocator == 0)return NULL;
...
bool pool_allocator_free(struct Pool_Allocator* allocator, void* ptr){
  uintptr_t p = (uintptr_t)ptr;
    ...
    assert(((p - allocator->base) % allocator->block_size) == 0);
  #endif
  ...
  if (((p - allocator->base) % allocator->block_size) != 0) return false;
